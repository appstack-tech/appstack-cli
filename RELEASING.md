...
Releases are driven by a `v`-prefixed Git tag. The release workflow verifies
the tag and package version, runs the full test suite, packs the package once,
publishes that tarball to npm, and attaches the same tarball and its SHA-256
...
The tag must exactly equal `v` plus the version in `package.json`, and its
commit must be part of `main`. GitHub-generated release notes cover the changes
since the previous release. Stable versions are published to npm's `latest`
distribution tag; semantic-version prereleases are published to `next` and
marked as prereleases on GitHub.

GitHub Release titles contain only the version number, without the tag's `v`
prefix.
